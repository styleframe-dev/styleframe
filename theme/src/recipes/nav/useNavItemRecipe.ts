 ...
 */
export const useNavItemRecipe = createUseRecipe(
	"nav-item",
	{
		base: {
			display: "inline-flex",
	...
	(s) => {
		...
		// Active state: apply semibold weight
		selector(".nav-item.-active", {
			fontWeight: "600",
		});

		// Disabled state via class modifier (supplements :disabled pseudo)
		selector(".nav-item.-disabled", {
			cursor: "not-allowed",
			opacity: "0.5",
			pointerEvents: "none",
		});
