...
/**
 * Popover header recipe with bottom separator.
 */
export const usePopoverHeaderRecipe = createUseRecipe(
	"popover-header",
	{
		base: {
			display: "flex",
	...
	(s) => {
		// Collapse bottom border when followed by another popover part
		s.selector(".popover-header:has(+ .popover-body)", {
			borderBottomColor: "transparent",
		});
		s.selector(".popover-header:has(+ .popover-footer)", {
			borderBottomColor: "transparent",
		});
	},
);
