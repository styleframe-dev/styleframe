...
import { useTextareaPrependRecipe } from "./useTextareaPrependRecipe";
...
describe("useTextareaPrependRecipe", () => {
	it("should create a recipe with correct metadata", () => {
		const s = createInstance();
		const recipe = useTextareaPrependRecipe(s);

		expect(recipe.type).toBe("recipe");
		expect(recipe.name).toBe("textarea-prepend");
	});
