...
8. **The `default` key generates the base variable name without suffix** - e.g., `useFontSize(s, { default: '1rem' })` returns `{ fontSize }` not `{ fontSizeDefault }`, and generates CSS variable `--font-size` not `--font-size--default`. This applies to all theme composables: `useColor` returns `color`, `useSpacing` returns `spacing`, etc.
