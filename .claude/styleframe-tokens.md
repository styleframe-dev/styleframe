...
const { colorPrimaryShade100 } = useColorShadeDesignTokens(s, colorPrimary, {
	"shade-100": 10,
});

// Tint variants (relative: raises L channel — lighter)
const { colorPrimaryTint100 } = useColorTintDesignTokens(s, colorPrimary, {
	"tint-100": 10,
});
```
...
// Font families
const { fontFamily, fontFamilyMono, fontFamilyPrint } =
...
const { boxShadowSm, boxShadowMd, boxShadowLg } = useBoxShadowDesignTokens(s, {
