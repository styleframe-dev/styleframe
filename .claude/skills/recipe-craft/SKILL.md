...
- Any recipe that registers `@keyframes` in its `setup()` callback MUST assert each registration in its colocated test via the shared `expectKeyframesRegistered(s, name)` helper (`theme/src/__tests__/expectKeyframesRegistered.ts`). An `animationName` is an opaque string to the base/variant object assertions, so a deleted `s.keyframes(...)` call silently makes the animation inert and nothing red catches it — the registration assertion is the only thing that does.
