...
Create fluid typography and spacing that scales smoothly across viewport sizes without breakpoints.

**Available composables:**
- **`useFluidTypography()`**: Generate responsive typography using the Utopia fluid type scale
...
selector('.heading', {
    fontSize: ref(fontSizeLg), // Scales smoothly from mobile to desktop
});
```

::warning
**Note**: Fluid design composables are currently under development.
::
