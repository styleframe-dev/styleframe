...
Previously every `font-size.*` token in a config that used fluid ranges (via `useFontSizeDesignTokens` or the preset's default fluid scale) shipped to Figma as a useless `STRING` variable holding the raw `calc(...)` formula. They now ship as spec-conformant `dimension` tokens (`{value, unit: "px"}`) that the Figma plugin renders as real numeric variables (`font-size/md = 18`, `font-size/lg = 22.5`, etc.).
