...
```ts
import { styleframe } from 'styleframe';
import { useUtilitiesPreset, useSpacingDesignTokens, useColorDesignTokens } from '@styleframe/theme';
...
- **Combine with modifiers**: Use [`useModifiersPreset()`](/docs/utilities/modifiers/presets) alongside `useUtilitiesPreset()` to register modifier factories for state-based variants
