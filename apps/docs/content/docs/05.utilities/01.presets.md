---
title: Presets
description: Quickly register all utility factories with the useDefaultUtilitiesPreset composable for seamless recipe integration and comprehensive utility class generation.
navigation:
...
The `useDefaultUtilitiesPreset` composable registers all available utility factories with your Styleframe instance in a single function call. It's essential when using [recipes](/docs/api/recipes) that need to auto-generate utility classes from CSS property values.
...
```ts
function useDefaultUtilitiesPreset(s: Styleframe): Record<string, UtilityCreator>
```
...
```ts
import { styleframe } from 'styleframe';
import { useDefaultUtilitiesPreset, useSpacing } from '@styleframe/theme';
...
```ts [styleframe.config.ts]
import { styleframe } from 'styleframe';
import { useDefaultDesignTokensPreset, useDefaultUtilitiesPreset } from '@styleframe/theme';

const s = styleframe();

// Set up design tokens
const { colors, spacing, borderRadius } = useDefaultDesignTokensPreset(s);
...
```ts
import { styleframe } from 'styleframe';
import { useDefaultUtilitiesPreset, useSpacing, useColor } from '@styleframe/theme';
...
- **Use with recipes**: Call `useDefaultUtilitiesPreset()` before defining recipes to enable auto-generation
...
- **Combine presets**: Use alongside `useDefaultDesignTokensPreset()` for a complete system
...
const { createOpacityUtility } = useDefaultUtilitiesPreset(s);
