...
See [Configuration Options Reference](#configuration-options-reference) for all available options.
...
All configuration options accept `false` to disable the utility or `Record<string, string>` to provide custom values. Omitting an option uses the preset defaults.
