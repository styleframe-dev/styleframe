...
When working with [recipes](/docs/api/recipes), you need to register utility factories before defining your recipes. The `useUtilities()` function provides a convenient way to register all available utility composables at once:
...
const utilities = useUtilities(s);
...
The `useUtilities()` function:
- **Registers all utility factories** with the Styleframe instance
- **Enables recipe auto-generation**: Recipes automatically create utility classes for properties used in `base`, `variants`, and `compoundVariants`
- **Returns creator functions**: You can optionally use the returned creators to define specific utility values

```ts [styleframe.config.ts]
// You can also use the returned creators to define utility values
const {
    createMarginUtility,
...
// Define specific values for utilities you want to use directly
createMarginUtility({ sm: '0.5rem', md: '1rem', lg: '1.5rem' });
createPaddingUtility({ sm: '0.5rem', md: '1rem', lg: '1.5rem' });
```

::tip
**When to use `useUtilities()`**: Use this when you're building components with recipes and want all CSS properties to be available for auto-generation. For simpler projects where you only need specific utilities, import and call individual utility composables instead.
::
...
**Available composables:**
- **`useMarginUtility()`**: Set margins (+ directional variants)
- **`usePaddingUtility()`**: Set padding (+ directional variants)
