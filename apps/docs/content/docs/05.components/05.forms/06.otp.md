...
<div
	class="otp _display:inline-flex ..."
	role="group"
	aria-label="One-time password"
>
	<input
		class="otp-cell _text-align:center ..."
		type="text"
		inputmode="numeric"
		maxlength="1"
	/>
...
The cell shares the Input recipe's color / style / state surface through an internal helper, so the two stay visually consistent. The difference is structural: an Input wraps a transparent field and uses a `:focus-within` ring, while each OTP cell _is_ the focusable `<input>` and uses a `:focus-visible` ring.
:::
