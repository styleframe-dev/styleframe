...
Set `aria-selected="true"` to apply a subtle tinted background, a medium font weight, and reveal the leading `.select-option-check` indicator. The check slot is always present in the markup but hidden until the option is selected.
...
| **Option** | `useSelectOptionRecipe()` | A `.select-option` row &mdash; hover / focus / active plus `aria-selected` and `aria-disabled` states |
...
<ul class="selectPanel(...)" role="listbox">
    ...
    <li class="selectOption(...)" role="option" aria-selected="true">
        <span class="select-option-check">✓</span>Apple
    </li>
