...
The Select recipes integrate directly with the default [design tokens preset](/docs/theme/design-tokens/presets) and generate type-safe utility classes at build time with zero runtime CSS.
...
::story-preview
---
story: theme-recipes-forms-select--country-select
panel: true
---
::
...
The Select is the visual union of three existing recipes, and each part stays faithful to its source. The trigger mirrors the [Input](/docs/components/forms/input) recipe (`solid`, `soft`, `ghost`), while the panel and options mirror the [Dropdown](/docs/components/overlays/dropdown) recipe (`solid`, `soft`, `subtle`). Pass `solid` or `soft` to keep both vocabularies aligned; use `ghost` on the trigger with `subtle` on the panel for a borderless look.
:::
