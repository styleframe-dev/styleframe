...
Input supports **inline** addons (`prefix`, `suffix`) that render inside the field and share its surface &mdash; use them for icons, currency symbols, and units. To attach an interactive block such as a button or select _outside_ the field, group the controls with a [Field Group](/docs/components/forms/field-group) instead.
