...
```ts [src/components/Accordion.tsx]
...
Creates the content recipe &mdash; a CSS grid wrapper that animates `grid-template-rows` from `0fr` to `1fr` when it has `data-state="open"`. Its single grid child is styled as an `overflow: hidden` clip so the padded body inside can collapse fully. Accepts the same parameters as `useAccordionRecipe`.
...
Creates the body recipe for the padded inner content. It sits inside the content recipe's overflow clip, so it carries only padding and typography (scaled by the `size` axis) &mdash; never `overflow` or `min-height`, which on the grid child would stop the panel from collapsing fully.
...
The content wrapper is a CSS grid with a single row. It animates `grid-template-rows` from `0fr` (collapsed) to `1fr` (expanded) when it carries `data-state="open"`. Its grid child is an `overflow: hidden` clip with `min-height: 0`, and the padded body sits inside that clip &mdash; so the row collapses to exactly zero (padding on the grid child would floor it at the padding height). You only toggle the `data-state` attribute; the browser interpolates the height with no measurement.
:::
