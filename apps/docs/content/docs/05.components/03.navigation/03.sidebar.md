...
No. The recipe ships the collapsed CSS, keyed off the `collapsed` axis (the `.-collapsed` class). Toggling it — via a button, a keyboard shortcut, a mobile drawer, or persisted state — is your application's responsibility.
:::
...
:::accordion-item{label="Which surface variant should I use?" icon="i-lucide-circle-help"}
The panel defaults to `solid`, which suits a sidebar that sits on its own background. If the panel shares the page background and would otherwise blend in, switch to `soft` (a faint tint) or `subtle` (a tint plus a border) so it reads as a distinct surface.
:::
