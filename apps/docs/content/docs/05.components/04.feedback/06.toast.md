...
The Toast recipe shares the [Callout](/docs/theme/components/callout) recipe's color, variant, size, and orientation vocabulary. The difference is presentation: a toast **floats**. It carries an elevation shadow and is sized to its content, where a callout is an inline, full-width surface. Everything else &mdash; the nine colors, four visual styles, three sizes, and two orientations &mdash; is identical.
