...
<template>
	...
	<div :class="toast({ variant, size, orientation })" role="status">
		...
		<div :class="toastContent()">
			...
			<p v-if="description" :class="toastDescription({ size })">{{ description }}</p>
			<slot />
		</div>
...
| Size | Font Size (title) | Description | Padding (V / H) | Gap |
|------|-----------|-------------|------------------|-----|
| `sm` | `@font-size.xs` | `@font-size.2xs` | `@0.5` / `@0.75` | `@0.5` |
| `md` | `@font-size.sm` | `@font-size.xs` | `@1` / `@1.25` | `@1` |
| `lg` | `@font-size.md` | `@font-size.sm` | `@1.25` / `@1.5` | `@1.25` |

The title inherits the toast's font size; the description tracks the same `size` axis one token below, so the title/description hierarchy holds at every size.
