...
<template>
	...
	<div :class="toast({ variant, size, orientation })" role="status">
		...
		<div :class="toastContent()">
			<strong v-if="title" :class="toastTitle({ size })">{{ title }}</strong>
...
|------|---------------------|------------------|-----|
| `sm` | `@font-size.sm` | `@0.5` / `@0.75` | `@0.5` |
| `md` | `@font-size.md` | `@1` / `@1.25` | `@1` |
| `lg` | `@font-size.lg` | `@1.25` / `@1.5` | `@1.25` |

Title and description share the same `size` axis on the literal token — `sm` → `sm`, `md` → `md`, `lg` → `lg` — so they render at the same font size at every step. The title stays dominant through weight (`semibold` vs the description's `normal`), not size.
...
| `useToastDescriptionRecipe()` | `toast-description` | Supporting line — same font size as the title, a step down in weight |
