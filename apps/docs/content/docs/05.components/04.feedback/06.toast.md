...
::story-preview
---
story: theme-recipes-feedback-toast--outline
panel: true
---
::
...
Light tinted background with colored text and a matching border. Combines the softness of the `soft` variant with the definition of `outline`. This is the **default variant** for the Toast recipe.
...
They share the same color, variant, size, and orientation vocabulary and the same 36 color-variant compound variants. The difference is presentation: the toast floats. Its base adds a `@box-shadow.md` elevation and omits the full-width `flex-basis` so it sizes to its content, where the callout is an inline, full-width surface. Use a callout for inline, in-flow messages and a toast for transient, floating notifications.
:::
