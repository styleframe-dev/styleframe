...
- **Read as a floating surface**: A built-in `@box-shadow.md` elevation lifts the toast off the page so it reads as a transient notification, not an inline notice.
...
// → "toast _display:flex _box-shadow:md _padding:md ..."
```

```html [src/components/toast.html]
<div class="toast _display:flex _box-shadow:md _padding:md ..." role="status">
...
The `useToastProgressRecipe()` composable adds a duration indicator &mdash; a thin line pinned to the toast's bottom edge that shrinks from full width to zero, mirroring the auto-dismiss countdown. It is pure CSS: the recipe owns the `toast-progress` keyframes and reads the timing from a `--toast-duration` custom property, so nothing has to flip a data-attribute at runtime.

The line uses `currentColor`, so it inherits each color and variant's foreground automatically. The parent `toast` recipe is `position: relative` with `overflow: hidden`, which anchors the line and clips it to the toast's rounded corners.
...
::note
**Reduced motion:** the line animates continuously toward dismissal. If you honor `prefers-reduced-motion`, pause or hide the indicator (and the auto-dismiss timer) so motion-sensitive users are not rushed.
::
...
|-----------|-------|---------|
| `useToastIconRecipe()` | `toast-icon` | Non-shrinking inline-flex wrapper for the leading icon |
| `useToastContentRecipe()` | `toast-content` | Main text area, fills the available space |
| `useToastProgressRecipe()` | `toast-progress` | Duration line pinned to the bottom edge; shrinks over `--toast-duration` (default `5s`) |
...
They share the same color, size, and orientation vocabulary. The toast trims the variant axis to the three filled styles — `solid`, `soft`, `subtle` — dropping callout's `outline`, since a transparent-background surface floating over arbitrary page content would be illegible. The other difference is presentation: the toast floats. Its base adds a `@box-shadow.md` elevation and omits the full-width `flex-basis` so it sizes to its content, where the callout is an inline, full-width surface. Use a callout for inline, in-flow messages and a toast for transient, floating notifications.
:::
