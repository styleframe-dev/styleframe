...
import { toast, toastContent, toastDescription, toastDismiss, toastIcon, toastTitle, type ToastProps } from "virtual:styleframe";
...
An opaque neutral card &mdash; white in light mode, near-black in dark mode &mdash; with a hairline border. The highest-contrast surface, matching Nuxt UI's default toast; ideal for critical notifications that demand immediate attention.
...
A faint neutral fill with a hairline border. Combines the softness of the `soft` variant with a defining border for a little more structure. This is the **default variant** for the Toast recipe.
