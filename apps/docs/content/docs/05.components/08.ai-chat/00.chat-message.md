...
::story-preview
---
story: theme-recipes-ai-chat-chatmessage--ghost
panel: true
---
::
...
Creates the chat-message content recipe &mdash; the bubble itself. Owns the 12-entry color &times; variant compound matrix (3 colors &times; 4 visual variants). The `ghost` variant strips bubble chrome regardless of color, so it has no compound entries. Accepts the same parameters as `useChatMessageRecipe`.
