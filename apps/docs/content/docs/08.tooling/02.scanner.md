...
<div class="_display:flex _padding:1rem _gap:0.5rem">
	...
	<button
		class="_background:primary _hover:background:secondary _padding:0.75rem"
	>
		Click me
	</button>
</div>
...
<template>
	<button
		class="_padding:md _background:primary _hover:background:secondary _font-weight:bold"
	>
		<slot />
	</button>
</template>
...
Yes. The `@styleframe/scanner` package provides a standalone programmatic API. Use `createScanner()` from `@styleframe/scanner/node` to integrate the scanner into any build tool or custom workflow. Browser-safe helpers (`quickScan`, `parseUtilityClass`, `matchUtilities`, `registerMatchedUtilities`, …) live on the main `@styleframe/scanner` entry.
:::
