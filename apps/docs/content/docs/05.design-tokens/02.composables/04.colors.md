...
The `useColorLightnessDesignTokens()` function creates a set of color variants at specific OKLCH lightness targets. Each level maps to an absolute lightness value (0–1), and chroma is automatically scaled to stay within the sRGB gamut.
...
```ts [styleframe.config.ts]
import { styleframe } from "styleframe";
import { useColorDesignTokens, useColorLightnessDesignTokens, defaultColorLightnessValues } from "@styleframe/theme";
...
const colorPrimaryLevels = useColorLightnessDesignTokens(s, colorPrimary, {
    ...defaultColorLightnessValues,
...
const colorPrimaryLevels = useColorLightnessDesignTokens(
    s,
    colorPrimary,
    defaultColorLightnessValues,
);
...
Lightness levels target absolute OKLCH lightness values (e.g., L = 0.75), while tints and shades make relative adjustments from the base color's lightness (e.g., 10% lighter or darker). Both are pre-computed at build time with gamut-aware chroma scaling.
:::
