...
```ts [styleframe.config.ts]
import { styleframe } from 'styleframe';
import { useColorDesignTokens, useColorLightnessDesignTokens, useBorderColorDesignTokens } from '@styleframe/theme';
...
const { colorGray300, colorGray500, colorGray700 } = useColorLightnessDesignTokens(s, colorGray, {
    300: 35,
    500: 55,
    700: 82,
} as const);
...
```ts [styleframe.config.ts]
import { styleframe } from 'styleframe';
import { useBorderStyleDesignTokens, useBorderWidthDesignTokens, useColorDesignTokens, useColorLightnessDesignTokens, useBorderColorDesignTokens } from '@styleframe/theme';
...
// Create lightness variants
const { colorGray300, colorGray700 } = useColorLightnessDesignTokens(s, colorGray, {
    300: 35,
    700: 82,
} as const);
...
Yes! You can combine `useBorderColorDesignTokens()` with composables like `useColorLightnessDesignTokens()`, `useColorShadeDesignTokens()`, and `useColorTintDesignTokens()` to create sophisticated border color systems with automatic variants. Just pass the manipulated color references to `useBorderColorDesignTokens()`.
:::
