...
The Button Group recipe integrates directly with the [Button recipe](/docs/theme/components/composables/button) and generates type-safe utility classes at build time with zero runtime CSS.
...
The Button Group works with all [Button color variants](/docs/theme/components/composables/button#colors). Each button within the group retains its own color styling while the group handles layout and border collapsing.
...
The Button Group showcases how each [Button visual style](/docs/theme/components/composables/button#variants) looks when buttons are joined together. Border collapsing is most visible with variants that have explicit borders, like `outline` and `subtle`.
...
The Button Group works with all [Button sizes](/docs/theme/components/composables/button#sizes). All buttons within a group should use the same size for consistent alignment.
...
- **Pair with the Button recipe**: The Button Group recipe targets child elements with the `.button` class. Always use it alongside the [Button recipe](/docs/theme/components/composables/button) for consistent border collapsing.
