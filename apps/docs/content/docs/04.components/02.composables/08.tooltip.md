...
The Tooltip recipes integrate directly with the default [design tokens preset](/docs/theme/design-tokens/presets) and generate type-safe utility classes at build time with zero runtime CSS.
...
- **Keep content concise.** Tooltips are for supplementary information, not essential content. If the information is critical, use an inline label or a [callout](/docs/theme/components/composables/callout) instead.
