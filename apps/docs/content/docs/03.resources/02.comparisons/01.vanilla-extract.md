...
Styleframe provides a dedicated `theme()` API that integrates with its variable system. Themes use added using `data-theme` or custom selectors, which simplifies switching and supports nested themes (e.g., a dark sidebar inside a light app). You can override both variables and selectors within a theme definition.
...
## Appendix: Common additions teams adopt with Vanilla Extract for design systems

Vanilla Extract can scale extremely well, and many teams layer in additional building blocks over time, such as:

* A **theme contract** + multiple themes (light/dark, brand themes)
* A standard approach for **variants** (often `@vanilla-extract/recipes`)
* A **utility/atomic layer** (often Sprinkles, or internal utilities)
* Team conventions for:

  * file layout (`theme.css.ts`, `tokens`, `components/*/*.css.ts`, etc.)
  * naming and scoping (global vs component vs utilities)
  * token governance (deprecations/migrations)

This can be a strong approach when you want a thin base and the freedom to assemble a system that matches your organization’s preferences.
