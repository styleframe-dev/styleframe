...
The `useFluid()` composable establishes the viewport range for all fluid calculations in your theme. It creates four essential CSS variables that serve as the foundation for fluid design tokens.
...
**Pro tip:** Call `useFluid()` once at the beginning of your theme configuration. All other fluid composables will automatically reference these viewport variables.
...
- **Call `useFluid()` once**: Set up your fluid viewport range once at the beginning of your theme configuration. Multiple calls will override previous values.
...
Create a custom breakpoint variable with a narrow viewport range. For mobile-only scaling, use `minWidth: 320, maxWidth: 767`. For desktop-only, use `minWidth: 1024, maxWidth: 1920`. Pass this custom breakpoint as the third argument to `useFluidClamp()`.
:::
