...
Here's a minimal example showing fluid typography in action:
...
const { fontSize, fontSizeLg, fontSizeXl, fontSize2xl } = useFluidFontSize(s,
    { min: 16, max: 18 },
    {
        default: { min: scaleMinPowers[0], max: scaleMaxPowers[0] },
        lg: { min: scaleMinPowers[1], max: scaleMaxPowers[1] },
...
{
    sm: { min: scaleMinPowers[-1], max: scaleMaxPowers[-1] },      // Smaller than base
    default: { min: scaleMinPowers[0], max: scaleMaxPowers[0] },   // Base size
    lg: { min: scaleMinPowers[1], max: scaleMaxPowers[1] },        // Larger than base
    xl: { min: scaleMinPowers[2], max: scaleMaxPowers[2] }         // Even larger
}
```
...
const fontSizeDisplay = variable(
    'font-size--display',
    useFluidClamp(s, {
        min: s.css`${s.ref(fontSizeDisplayMin)} * ${scaleDisplayMinPowers[0]}`,
        max: s.css`${s.ref(fontSizeDisplayMax)} * ${scaleDisplayMaxPowers[0]}`
    })
);
...
const fontSizeDisplayLg = variable(
    ...
    useFluidClamp(s, {
        min: s.css`${s.ref(fontSizeDisplayMin)} * ${scaleDisplayMinPowers[1]}`,
        max: s.css`${s.ref(fontSizeDisplayMax)} * ${scaleDisplayMaxPowers[1]}`
    })
);
...
const { fontSizeLg, fontSizeXl } = useFontSize(s, {
    ...
    xl: useFluidClamp(s, { min: fontSizeMinXl, max: fontSizeMaxXl })
});
```
...
const { fontSize } = useFluidFontSize(s, { min: 16, max: 18 }, {
    default: { min: scaleMinPowers[0], max: scaleMaxPowers[1] }  // 0 doesn't match 1!
});

// ✅ Good: Matched powers
const { fontSize } = useFluidFontSize(s, { min: 16, max: 18 }, {
    default: { min: scaleMinPowers[0], max: scaleMaxPowers[0] }  // Both use power 0
});

// ✅ Also good: Consistent progression
const { fontSizeLg } = useFluidFontSize(s, { min: 16, max: 18 }, {
    lg: { min: scaleMinPowers[1], max: scaleMaxPowers[1] }  // Both use power 1
});
```
