...
import { useFluid, useFluidFontSize } from '@styleframe/pro';
...
**Pro tip:** Always call `useFluid()` before using `useFluidFontSize()`. The fluid font size composable relies on the fluid breakpoint variables created which are picked up automatically.
...
**3. Custom breakpoint** (optional) - By default, references the `fluidBreakpoint` variable from `useFluid()`. You can override the default fluid breakpoint:
...
- **Always call `useFluid()` first**: The other fluid composables depend on fluid breakpoint variables.
...
The default range of 320px to 1440px works well for most projects, accommodating the smallest mobile devices up to most desktop monitors. You can customize this with `useFluid(s, { minWidth: 375, maxWidth: 1920 })` if needed. The key is choosing a range that matches your actual user viewport distribution.
:::
...
- **[Clamp Function](/docs/design-tokens/fluid-design/viewport)**: Learn about `useFluid()` and `useFluidClamp()`
