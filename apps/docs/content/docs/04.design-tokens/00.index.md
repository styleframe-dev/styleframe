...
For rapid setup, Styleframe provides the `useDefaultDesignTokensPreset()` composable that creates a complete design token system with sensible defaults in a single function call.
...
// Set up a complete design token system with defaults
const tokens = useDefaultDesignTokensPreset(s);

// Access all generated tokens
const { colors, spacing, fontSize, fontWeight, borderRadius, boxShadow } = tokens;

export default s;
```

The preset generates tokens for all major design token categories:

| Category | Default Values |
|----------|----------------|
| **Colors** | Primary, secondary, success, warning, danger, info, light, dark (with lightness, shade, and tint variations) |
| **Spacing** | xs, sm, md, lg, xl scale |
| **Typography** | Font families (base, mono), sizes, weights, line heights, letter spacing |
| **Borders** | Border widths, radiuses, and styles |
| **Shadows** | sm, md, lg, xl elevation levels |
| **Scale** | Modular scale with configurable ratio |
| **Breakpoints** | sm, md, lg, xl responsive breakpoints |
| **Easing** | Common animation timing functions |

[Learn more about Presets →](/docs/design-tokens/presets)
...
**Available composables:**
- **`useEasing()`**: Create easing variables including CSS keywords, cubic-bezier curves, and linear() functions for spring and bounce effects
...
const { easing, easingSpring, easingBounce } = useEasing(s);
```
...
| **Easing** | `useEasing` | Animation timing functions |
