...
::note
**Want Tailwind's short names?** Swap `useUtilitiesPreset` for [`useShorthandUtilitiesPreset`](/docs/theme/utilities/preset#useshorthandutilitiespreset) in Step 1 and the same utilities generate as `_p:md`, `_m:sm`, `_rounded:lg` instead of `_padding:md`, `_margin:sm`, `_border-radius:lg` — only the names change; grammar and tokens stay put. Read the tables below as a mechanical name swap.
::
