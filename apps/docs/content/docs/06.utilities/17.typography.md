...
**Part of the Utilities Preset:** The utilities on this page are registered by the [Utilities Preset](/docs/theme/utilities/presets) (`useUtilitiesPreset`). For most projects, applying them via the preset is the recommended approach.
::
...
const { fontSizeXs, fontSizeSm, fontSize, fontSizeLg, fontSizeXl, fontSize2xl, fontSize3xl } = useFontSizeDesignTokens(s, {
    xs: '0.75rem',
    sm: '0.875rem',
    default: '1rem',
    lg: '1.125rem',
...
._font-size { font-size: var(--font-size); }
...
<p class="_font-size">Default body text</p>
...
Font size utilities inherit fluid behavior automatically — pass `[min, max]` pixel ranges to `useFontSizeDesignTokens()` and every utility class that references those tokens picks up the `clamp()` math at no extra cost.
...
const { fontSizeSm, fontSizeMd } = useFontSizeDesignTokens(s, {
    min: 16,        // emits --font-size--min (configurable fluid base)
    max: 18,        // emits --font-size--max
    sm: '0.875rem', // fixed
    md: ['@font-size.min', '@font-size.max'],   // fluid: clamp 16px → 18px
} as const);
...
:root {
    ...
    --font-size--max: 18;
    --font-size--sm: 0.875rem;
    --font-size--md: calc((var(--font-size--min) / 16 * 1rem) + (var(--font-size--max) - 16) * var(--fluid--breakpoint));
}

._font-size\:sm { font-size: var(--font-size--sm); }
._font-size\:md { font-size: var(--font-size--md); }
```
...
::tip
See [Fluid Typography](/docs/design-tokens/composables/fluid-design/typography) for pairing fluid ranges with modular scales and the full `useFontSizeDesignTokens()` API.
::
...
const { colorGray900, colorGray500, colorPrimary } = useColorDesignTokens(s, {
    gray900: '#111827',
    gray500: '#6b7280',
    primary: '#006cff',
} as const);
...
<p class="_font-size _font-weight:normal _line-height:normal">
