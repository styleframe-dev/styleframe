...
// When you define a recipe, utilities auto-generate from property values
s.recipe({
	name: "button",
	base: {
		display: "inline-flex",
...
- `Record<string, string>`: Provide custom values, which merge with defaults by default
- With `meta.merge: false`: Replace defaults entirely with the custom record
...
`useShorthandUtilitiesPreset` delegates to `useUtilitiesPreset` and inherits the same merge default.

#### Meta

| Option       | Type      | Description                                                                                               |
| ------------ | --------- | --------------------------------------------------------------------------------------------------------- |
| `meta.merge` | `boolean` | Defaults to `true`. Set to `false` to replace defaults entirely with the custom record instead of merging |
...
const { createMarginUtility, createPaddingUtility, createGapUtility } =
	useUtilitiesPreset(s);
