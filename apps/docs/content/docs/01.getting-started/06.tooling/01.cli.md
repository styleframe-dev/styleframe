...
Export Styleframe variables to spec-conformant DTCG (Design Tokens Community Group) JSON. The output is consumed by the `@styleframe/figma` plugin (which performs the DTCG → Figma value conversion) and any other DTCG-compatible tooling.
::
::
...
Generate Styleframe TypeScript code from DTCG JSON produced by Figma (via the `@styleframe/figma` plugin's Export action or the Variables REST API).
::
::
...
The `dtcg` and `figma` commands handle each direction:

1. **Code → Figma**: Run `styleframe dtcg export` to generate a spec-conformant DTCG JSON file, then import it into Figma using the `@styleframe/figma` plugin
2. **Figma → Code**: Export your Figma variables as DTCG JSON via the plugin, then run `styleframe figma import -i tokens.json` to generate Styleframe code
