...
```bash
styleframe dtcg import -i tokens.json --output src/tokens.styleframe.ts
styleframe dtcg import -i tokens.json -o src/tokens.styleframe.ts
```
...
```bash
styleframe dtcg import -i tokens.json --rem
styleframe dtcg import -i tokens.json --rem --baseFontSize 14
```
...
```bash
styleframe dtcg import -i tokens.json --composables false
```
...
2. **Figma → Code**: Export your Figma variables as DTCG JSON via the plugin, then run `styleframe dtcg import -i tokens.json` to generate Styleframe code
