...
::warning
**Responsive prefixes are not built in.** Styleframe does not ship Tailwind's `sm:`/`md:`/`lg:` breakpoint modifiers by default. You define them yourself as a multi-key modifier over your breakpoint tokens — which also means you control the exact query. For example:
...
const breakpointUp = modifier(
    ['sm', 'md', 'lg'],
    ({ key, declarations }) => ({
        [`@media (min-width: ${breakpoints[key as keyof typeof breakpoints]})`]: declarations,
    }),
);
...
Applied to a utility, this generates `_md:padding:xl` and friends. See [Utility Modifiers](/docs/api/utility-modifiers) for the full API.
::
