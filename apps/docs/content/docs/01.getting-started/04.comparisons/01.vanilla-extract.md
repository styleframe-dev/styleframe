...
| Architecture        | Transpiler-first (dual CSS + TS output)      | CSS extraction from style objects              |
...
| Output              | Static CSS + typed TypeScript runtime        | Static CSS extracted from `.css.ts`            |
| Best for            | Large design systems, multi-team consistency | Teams wanting a thin, flexible base            |

## Architectural differences

The most fundamental difference between Styleframe and Vanilla Extract is **how they generate output**.

**Vanilla Extract** extracts CSS from your `.css.ts` files at build time. The TypeScript you write _is_ the source of truth - style objects become CSS, and class name references are typed because they come from those same objects. This is elegant and works well, but it means the output format is fixed: you get CSS and typed class name exports.

**Styleframe** takes a different approach: it's built as a **transpiler-first system**. Your TypeScript is first tokenized into an internal AST (Abstract Syntax Tree), and then transpile functions generate the final output. This architecture enables:

- **Dual output**: From one `.styleframe.ts` file, you get both static CSS _and_ typed TypeScript runtime functions (e.g., recipe functions with full IDE autocomplete).
- **Customizable output formats**: Recipes can be transpiled to follow a CVA-like API, a Vanilla Extract-style output, or something entirely custom - depending on your needs.
- **Extensible consumers**: The transpiler uses a consumer pattern, so you could add custom output formats (JSON for documentation, design token exports, etc.) without changing your source.

In practice, this means Styleframe can adapt to your team's conventions rather than requiring you to adapt to it. You could even use the generated tokens to render documentation for your design system components.
...
A practical heuristic: if you find yourself needing **tokens + themes + recipes + atomic utilities** early, an integrated toolkit like styleframe will reduce the number of separate building blocks you need to adopt and document.
...
- You plan to use your design system code for **more than just CSS** - like generating documentation or design system specs.
- You want complete control and customization over **how your code is output** (naming conventions, output formats) without forking or workarounds.
