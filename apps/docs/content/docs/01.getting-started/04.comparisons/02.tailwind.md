...
// One modifier per breakpoint, each closing over its own query
const breakpoints = { sm: "576px", md: "768px", lg: "992px" };
const breakpointsMax = Object.entries(breakpoints).map(([name, width]) =>
    modifier(`max-${name}`, ({ declarations }) => ({
        [`@media screen and (max-width: ${width})`]: declarations,
    })),
);
