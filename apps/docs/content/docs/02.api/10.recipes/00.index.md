...
Before defining recipes, you need to register the utility factories that will power the auto-generation of utility classes from your recipe declarations. The easiest way to do this is with `useDefaultUtilitiesPreset()`:
...
When you use a CSS property like `padding` or `display` in your recipe declarations, Styleframe automatically creates the corresponding utility classes (e.g., `_padding:[1rem]`, `_display:flex`). The `useDefaultUtilitiesPreset()` function registers all the utility factories needed for this auto-generation to work.

::tip
**Why register utilities?** Recipe declarations are converted to utility class names at runtime. For this to work, Styleframe needs to know how each CSS property maps to its utility class format. `useDefaultUtilitiesPreset()` registers all standard CSS property mappings.
::
