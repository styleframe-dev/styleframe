...
selector('.animated-element', {
    animation: css`${fadeInKeyframes} 0.3s ease-out`,
});
...
selector('.interactive-button', {
    backgroundColor: ref(colorPrimary),
    transition: css`all ${animationDurationNormal} ease`,

    '&:hover': {
        animation: css`${buttonHoverKeyframes} ${animationDurationFast} ease-out`,
    },
});
...
const s = styleframe();
const { keyframes, selector, media, ref, css } = s;
...
selector('.modal', {
    animation: css`${slideInMobileKeyframes} 0.3s ease-out`,
});

media('(min-width: 768px)', ({ selector }) => {
    selector('.modal', {
        animation: css`${slideInDesktopKeyframes} 0.4s ease-out`,
    });
});
...
selector('.animate-pulse', {
    animation: css`${pulseKeyframes} 2s cubic-bezier(0.4, 0, 0.6, 1) infinite`,
});

selector('.animate-spin', {
    animation: css`${spinKeyframes} 1s linear infinite`,
});

selector('.animate-shake', {
    animation: css`${shakeKeyframes} 0.82s cubic-bezier(0.36, 0.07, 0.19, 0.97)`,
});
...
selector('.progress-bar', {
    ...
    '&.active': {
        animation: css`${expandWidthKeyframes} 2s ease-out forwards`,
    },
});
...
selector('.notification', {
    ...
    '&.show': {
        animation: css`${fadeInUpKeyframes} 0.4s ease-out forwards`,
    },

    '&.hide': {
        animation: css`${fadeInUpKeyframes} 0.4s ease-out reverse`,
    },
});
...
selector('.bouncing-button', {
    animation: css`${bounceKeyframes} 0.6s infinite`,
});
