...
const transformTranslateUp = variable(
...
selector(".interactive-button", {
	...
	"&:hover": {
		animation: css`
			${buttonHoverKeyframes} ${animationDurationFast} ease-out
		`,
	},
});
...
selector(".modal", {
	animation: css`
		${slideInMobileKeyframes} 0.3s ease-out
	`,
});
...
media("(min-width: 768px)", ({ selector }) => {
	selector(".modal", {
		animation: css`
			${slideInDesktopKeyframes} 0.4s ease-out
		`,
	});
});
...
selector(".animate-shake", {
	animation: css`
		${shakeKeyframes} 0.82s cubic-bezier(0.36, 0.07, 0.19, 0.97)
	`,
});
...
selector(".progress-bar", {
	...
	"&.active": {
		animation: css`
			${expandWidthKeyframes} 2s ease-out forwards
		`,
	},
});
...
selector(".notification", {
	...
	"&.show": {
		animation: css`
			${fadeInUpKeyframes} 0.4s ease-out forwards
		`,
	},

	"&.hide": {
		animation: css`
			${fadeInUpKeyframes} 0.4s ease-out reverse
		`,
	},
});
