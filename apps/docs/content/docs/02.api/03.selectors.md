...
The `@` prefix is a shorthand for the `ref()` function — `"@color.primary"` is equivalent to `ref('color.primary')`. It resolves the token path to a CSS custom property reference at compile time.

::note
The `@` syntax validates that the referenced variable exists at the root level of the Styleframe instance. If the variable is not defined, an error will be thrown at compile time. Variables defined inside selectors or other nested contexts are not included in this validation.
::
