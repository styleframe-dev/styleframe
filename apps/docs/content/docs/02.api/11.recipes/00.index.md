...
Before defining recipes, you need to register the utility factories that will power the auto-generation of utility classes from your recipe declarations. The easiest way to do this is with `useUtilitiesPreset()`:
...
When you use a CSS property like `padding` or `display` in your recipe declarations, Styleframe automatically creates the corresponding utility classes (e.g., `_padding:[1rem]`, `_display:flex`). The `useUtilitiesPreset()` function registers all the utility factories needed for this auto-generation to work.

::tip
**Why register utilities?** Recipe declarations are converted to utility class names at runtime. For this to work, Styleframe needs to know how each CSS property maps to its utility class format. `useUtilitiesPreset()` registers all standard CSS property mappings.
::
...
### Compound Variant className

In addition to `css` declarations, compound variants can specify a `className` string that gets appended to the output when the conditions match. This is useful for adding non-utility classes like BEM modifiers or third-party class names:
...
recipe({
    ...
    compoundVariants: [
        {
            // className only — no css declarations needed
            match: { disabled: "true" },
            className: "button--disabled",
        },
        {
            // Both css and className together
...
// Output: "button _opacity:50 _cursor:[not-allowed] button--disabled"

button({ color: "primary", disabled: "false" })
// Output: "button _background:primary ... _hover:background:primary-dark button--interactive"
```

Both `css` and `className` are optional — a compound variant can have either one or both.
