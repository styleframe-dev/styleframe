...
Use `@` prefix to reference a token by its path. The token name becomes the class suffix. The referenced variable must exist at the root level — an error is thrown at compile time if it doesn't.
...
._hover\:background\:primary-dark:hover {
...
The type declaration file (`.d.ts`) contains the fully expanded type as top-level exports:
...
These exports are wired to the `virtual:styleframe` import specifier through a `paths` entry in your project's `tsconfig.json` (added automatically by `styleframe init`):
...
The companion `.styleframe/shims.d.ts` declares the `virtual:styleframe.css` module (picked up via the `.styleframe/**/*.d.ts` include).
