...
No. The `@styleframe/plugin` automatically handles `.styleframe.ts` files. Just create your file and import it with either `?css` (for styles) or `?ts` (for recipe functions).
:::
