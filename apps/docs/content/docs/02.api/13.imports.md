...
::note
**Shared modules are cached automatically**

The Styleframe plugin uses a serialized loader with module caching, so shared dependencies (like a `useTokens()` helper) are only compiled once and reused across all `.styleframe.ts` files. This prevents duplicate CSS output and improves build performance.

However, each `.styleframe.ts` file still creates its own `styleframe()` instance, so if your shared module registers variables on the passed instance, those variables will be registered once per file that calls it. For truly global tokens, consider defining them in your `styleframe.config.ts` instead.
::
