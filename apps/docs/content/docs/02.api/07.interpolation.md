...
selector('.button', {
    padding: css`${spacing} calc(${spacing} * 2)`,
});
...
selector('.text', {
    font: css`${fontSize}/${lineHeight} ${fontFamily}`,
    textShadow: css`1px 1px 2px rgba(0, 0, 0, 0.1), 0 0 1em ${fontSize}`,
});
...
.text {
    font: var(--font-size)/var(--line-height--normal) var(--font-family--sans);
    text-shadow: 1px 1px 2px rgba(0, 0, 0, 0.1), 0 0 1em var(--font-size);
}
```
...
::warning
Do not use interpolation for simple static variable strings such as `var(--example)`. While there is no runtime cost, interpolated values can limit certain compile-time optimizations that Styleframe applies to tokenized values.
::
...
selector('.gradient-bg', {
    background: css`linear-gradient(135deg, ${primaryColor} 0%, ${secondaryColor} 50%, ${accentColor} 100%)`,
    boxShadow: css`0 10px 20px rgba(0, 0, 0, 0.1), 0 0 0 1px ${primaryColor}`,
});

selector('.color-scheme', {
    color: ref(primaryColor),
    borderColor: css`color-mix(in srgb, ${primaryColor} 50%, ${secondaryColor})`,
    backgroundColor: css`oklch(from ${primaryColor} calc(l + 0.4) c h / 0.1)`,
});
...
const spinKeyframes = keyframes('spin', {
    ...
    '100%': {
        transform: css`rotate(${rotationAngle})`,
    },
});
...
selector('.interactive-card', {
    ...
    transition: css`all ${animationDuration} cubic-bezier(0.4, 0, 0.2, 1)`,

    '&:hover': {
        transform: css`scale(${transformScale}) rotate(${rotationAngle})`,
        animation: css`${spinKeyframes} ${animationDuration} ease-in-out`,
    },
});
...
selector('.responsive-grid', {
    display: 'grid',
    gridTemplateColumns: css`repeat(${gridColumns}, 1fr)`,
    gap: ref(gridGap),
    padding: css`clamp(1rem, 4vw, ${gridGap} * 2)`,
});

selector('.grid-item', {
    gridColumn: css`span min(4, ${gridColumns})`,
});

media(css`(min-width: ${breakpointMd})`, ({ selector }) => {
    selector('.grid-item', {
        gridColumn: css`span min(6, ${gridColumns})`,
    });
});
...
selector('.main-content', {
    width: css`calc(${containerWidth} - ${sidebarWidth} - ${spacing} * 2)`,
    marginLeft: css`clamp(${spacing}, 5vw, calc(${spacing} * 2))`,
    minHeight: css`max(100vh - ${spacing} * 4, 400px)`,
});
...
Interpolation has no runtime performance cost: it happens entirely at build time, generating clean CSS output. However, interpolated values that aren't tokenized may limit some compile-time optimizations. For simple static values, prefer using direct variable references instead of interpolation.
:::
