...
selector(".interactive-card", {
	...
	"&:hover": {
		...
		animation: css`
			${spinKeyframes} ${animationDuration} ease-in-out
		`,
	},
});
...
.interactive-card {
	...
	&:hover {
		transform: scale(var(--transform--hover-scale))
...
.main-content {
	width: calc(
		var(--container--max-width) - var(--sidebar--width) - var(--spacing--lg) * 2
	);
