...
The Badge recipe includes 9 color variants: 6 semantic colors (`primary`, `secondary`, `success`, `info`, `warning`, `danger`) plus 3 neutral-scale colors (`light`, `dark`, `neutral`). Each color is combined with every visual style variant through compound variants, so you get consistent, predictable styling across all combinations.
...
- **Choose colors by meaning, not appearance**: Use `success` for positive states and `danger` for errors &mdash; this keeps your UI consistent when tokens change.
