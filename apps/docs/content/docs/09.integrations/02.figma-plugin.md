...
Go to the [Styleframe Sync plugin](https://www.figma.com/community/plugin/1597716416772135340/styleframe){target="_blank"} page on the Figma Community.
...
styleframe dtcg export -c src/styleframe.config.ts -o design-tokens.json
...
```bash
styleframe dtcg import --input <file> [options]
```
...
# Custom output path
styleframe dtcg import -i tokens.json -o src/theme/tokens.ts

# With rem units
styleframe dtcg import -i tokens.json --rem --baseFontSize 16
...
- **Export tokens in CI/CD**: Automate `styleframe dtcg export` on every commit to generate fresh token files
