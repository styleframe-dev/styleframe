...
The `useScaleDesignTokens()` function creates a set of predefined scale ratio variables based on musical intervals and the golden ratio. It also creates a default `scale` variable that references one of the predefined scales.
...
The `scale` variable is a convenient reference to your chosen default scale. By default, it references `--scale--minor-third`, but you can customize this by passing a second argument to `useScaleDesignTokens()`.
...
The `useScalePowersDesignTokens()` function generates size multipliers by raising a scale to different powers, creating a systematic sizing system.
...
**Good to know:** While `useScalePowersDesignTokens()` generates the multipliers, you'll typically want to use them with [`useMultiplierDesignTokens()`](#usemultiplierdesigntokens) to create properly named variables for your design system.
...
The `useMultiplierDesignTokens()` function creates a set of variables by multiplying a base variable by different multipliers. This is perfect for combining with `useScalePowersDesignTokens()` to create complete size scales.
...
The `useMultiplierDesignTokens()` function automatically:
...
**Pro tip:** Using `useMultiplierDesignTokens()` with scale powers keeps your design system flexible. Change the base `fontSize` or `scale` variable, and all derived sizes update automatically throughout your design system.
...
:::accordion-item{label="Can I change the scale after calling useScaleDesignTokens()?" icon="i-lucide-circle-help"}
