...
.modal {
	transition:
		opacity var(--duration--normal) var(--easing--ease-out-cubic),
		transform var(--duration--normal) var(--easing--ease-out-cubic);
}
```
...
const { duration, durationMicro, durationUi, durationPage } =
	useDurationDesignTokens(s, {
		default: "@duration.ui",
...
const { durationFast, durationNormal, durationSlower } =
	useDurationDesignTokens(s);
const { easingEaseOut, easingEaseInOut, easingSpring } =
