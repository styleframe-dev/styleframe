...
**Part of the Design Tokens Preset:** `useBorderWidthDesignTokens`, `useBorderStyleDesignTokens`, and `useBorderColorDesignTokens` are included in the [Design Tokens Preset](/docs/theme/design-tokens/presets) (`useDesignTokensPreset`) and you can configure them through the preset's `borderWidth`, `borderStyle`, and `borderColor` options. For most projects, applying them via the preset is the recommended approach.
::
