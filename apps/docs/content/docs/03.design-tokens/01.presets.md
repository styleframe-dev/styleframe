...
| `fontSize` | `false \| Record<string, TokenValue>` | Static font size tokens (auto-disabled when `fluidFontSize` is active — pass `fluidFontSize: false` to use these) |
...
| `fluidFontSize` | `false \| true \| { base?, values? }` | Viewport-responsive font-size scale. **Enabled by default**; depends on `scale` and `fluidViewport`. Pass `false` to fall back to static `fontSize` (see [Fluid Typography](#fluid-typography)) |
...
Fluid typography is **enabled by default**. The preset emits `fluid.*` viewport variables and a viewport-responsive `font-size.*` scale built on top of the [fluid design composables](/docs/theme/design-tokens/composables/fluid-design). Every `font-size.*` variable becomes a `calc()` that interpolates between a min and max value as the viewport scales — the static `fontSize` domain is automatically replaced. To opt out and use static font sizes instead, pass `fluidFontSize: false`.
...
| _(per power)_ | `1`, `var(--scale--min)`, `var(--scale--min) * var(--scale--min)`, … | _(internal)_ | `--scale--min-powers--<n>` |
| _(per power)_ | `1`, `var(--scale--max)`, `var(--scale--max) * var(--scale--max)`, … | _(internal)_ | `--scale--max-powers--<n>` |
...
`fluidFontSize` interpolates each key between `base.min × multiplier[0]` and `base.max × multiplier[1]` (in px, output as `rem`). By default, the multipliers are scale-power references — each named step maps to `scale.min-powers.<n>` and `scale.max-powers.<n>`, where `n` is the step index. The min/max ratios come from [`fluidScale`](#default-fluid-scale).

Default base: `{ min: 16, max: 18 }` → emits `--font-size--min: 16` and `--font-size--max: 18`.

| Key | Step | Multipliers (default) | Variable | Output |
|-----|------|------------------------|----------|--------|
| `default` | — | `@font-size.md` | `fontSize` | `--font-size` |
| `3xs` | `-4` | `[@scale.min-powers.-4, @scale.max-powers.-4]` | `fontSize3xs` | `--font-size--3xs` |
| `2xs` | `-3` | `[@scale.min-powers.-3, @scale.max-powers.-3]` | `fontSize2xs` | `--font-size--2xs` |
| `xs` | `-2` | `[@scale.min-powers.-2, @scale.max-powers.-2]` | `fontSizeXs` | `--font-size--xs` |
| `sm` | `-1` | `[@scale.min-powers.-1, @scale.max-powers.-1]` | `fontSizeSm` | `--font-size--sm` |
| `md` | `0` | `[@scale.min-powers.0, @scale.max-powers.0]` | `fontSizeMd` | `--font-size--md` |
| `lg` | `1` | `[@scale.min-powers.1, @scale.max-powers.1]` | `fontSizeLg` | `--font-size--lg` |
| `xl` | `2` | `[@scale.min-powers.2, @scale.max-powers.2]` | `fontSizeXl` | `--font-size--xl` |
| `2xl` | `3` | `[@scale.min-powers.3, @scale.max-powers.3]` | `fontSize2xl` | `--font-size--2xl` |
| `3xl` | `4` | `[@scale.min-powers.4, @scale.max-powers.4]` | `fontSize3xl` | `--font-size--3xl` |
| `4xl` | `5` | `[@scale.min-powers.5, @scale.max-powers.5]` | `fontSize4xl` | `--font-size--4xl` |

You can override individual steps with literal multiplier tuples (e.g., `lg: [1.5, 2]`) — those bypass the scale entirely. See [Customizing Fluid Typography](#customizing-fluid-typography) for runnable examples and the hybrid (per-key) pattern.
...
::tabs
:::tabs-item{icon="i-lucide-code" label="Defaults"}
...
// Defaults — viewport range 320–1440px, base 16–18px, `font-size.*` becomes
// calc() expressions that scale with the viewport. No config required.
const preset = useDesignTokensPreset(s);
...
const preset = useDesignTokensPreset(s, {
    ...
    // The per-step `values` are optional — by default each named step is a
    // reference into the scale powers above. Use this only when you need to
    // override individual steps with literal multipliers.
    fluidFontSize: {
        base: { min: 16, max: 18 },
    },
});
...
:::
:::tabs-item{icon="i-lucide-circle-slash" label="Disable fluid"}
...
Use **fluid** (the default) when you want type to scale smoothly with the viewport so a single value reads well on both mobile and desktop without media queries. This is the [Utopia](https://utopia.fyi/) approach.
...
Use the **hybrid** pattern (`fluidFontSize: false` + `fluidViewport: true` + `useFluidClamp` inline on specific keys) when only a few values — typically display headings — need to scale fluidly. See the [Customizing Fluid Typography example](#customizing-fluid-typography).
