...
Default base: `{ min: 16, max: 18 }`. The preset bakes these absolute pixel values directly into each fluid `font-size.*` `calc()` expression — they are not emitted as standalone variables.
