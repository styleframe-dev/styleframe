...
You can override individual steps with literal pixel range tuples (e.g., `lg: [18, 22]`) — those bypass the scale entirely. See [Customizing Fluid Typography](#customizing-fluid-typography) for runnable examples and the hybrid (per-key) pattern.
