...
| `fluidFontSize` | `false \| true` | Viewport-responsive font-size scale. **Enabled by default**; depends on `scale` and `fluidViewport`. Pass `false` to fall back to static `fontSize` (see [Fluid Typography](#fluid-typography)) |
...
When `fluidFontSize` is enabled (the default), each key interpolates between a min and max pixel value using scale-power references — each named step maps to `scale.min-powers.<n>` and `scale.max-powers.<n>`, where `n` is the step index. The min/max ratios come from [`fluidScale`](#default-fluid-scale).

The default base pixel range is `16–18`. These values are baked directly into each fluid `font-size.*` `calc()` expression — not emitted as standalone variables. To use a different base, pass explicit `[min, max]` tuples via `fontSize` (e.g. `fontSize: { md: [14, 18] }`).
