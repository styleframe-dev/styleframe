...
The viewport composables provide the foundation for fluid responsive design in Styleframe. They handle the mathematical calculations that make values scale smoothly between minimum and maximum viewport widths, creating truly fluid interfaces that adapt seamlessly to any screen size. Past the maximum viewport width, fluid values are capped at their maximum so they never extrapolate beyond the intended ceiling.
...
The `useFluidViewportDesignTokens()` composable establishes the viewport range for all fluid calculations in your theme. It creates four essential CSS variables that serve as the foundation for fluid design tokens.
...
**Pro tip:** Call `useFluidViewportDesignTokens()` once at the beginning of your theme configuration. All other fluid composables will automatically reference these viewport variables.

::

::note

**Capping at `maxWidth`:** `useFluidViewportDesignTokens()` automatically registers a media query at `min-width: <maxWidth>` that pins `--fluid--screen` to `calc(var(--fluid--max-width) * 1px)`. This prevents all fluid clamp values from extrapolating beyond their intended ceiling once the viewport exceeds `maxWidth`. The `maxWidth` option accepts both numeric pixel values (`1440`) and CSS string values (`"90rem"`).
...
Create a fluid value by providing minimum and maximum endpoints. Ranges accept either an object `{ min, max }` or a tuple `[min, max]` — pick whichever reads better at the call site.
...
- **Call `useFluidViewportDesignTokens()` once**: Set up your fluid viewport range once at the beginning of your theme configuration. Multiple calls will override previous values.
...
- **Use `useFluidFontSizeDesignTokens()` for typography**: For complete type systems with modular scales, use `useFluidFontSizeDesignTokens()` instead of creating individual font size clamps. `useDesignTokensPreset` wires this up by default — pass `fluidFontSize: false` to opt out.
...
Create a custom breakpoint variable with a narrow viewport range using `useFluidViewportDesignTokens()`. For mobile-only scaling, use `minWidth: 320, maxWidth: 767`. For desktop-only, use `minWidth: 1024, maxWidth: 1920`. Pass this custom breakpoint as the third argument to `useFluidClamp()`.
:::
