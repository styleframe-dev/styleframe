...
To preserve a live link to a modular scale (so changing the scale changes every fluid font size), build the ranges from CSS expressions that multiply the base by `@scale.min-powers.<n>` / `@scale.max-powers.<n>`. The default `useDesignTokensPreset` configuration uses this exact pattern, so the live link survives any later override of `useScaleDesignTokens` or `useScalePowersDesignTokens`.
...
useFontSizeDesignTokens(s, {
    default: '@font-size.md',
    xs: [css`16 * ${ref('scale.min-powers.-2')}`, css`18 * ${ref('scale.max-powers.-2')}`],
    sm: [css`16 * ${ref('scale.min-powers.-1')}`, css`18 * ${ref('scale.max-powers.-1')}`],
    md: [16, 18],
    lg: [css`16 * ${ref('scale.min-powers.1')}`, css`18 * ${ref('scale.max-powers.1')}`],
    xl: [css`16 * ${ref('scale.min-powers.2')}`, css`18 * ${ref('scale.max-powers.2')}`],
});
```
