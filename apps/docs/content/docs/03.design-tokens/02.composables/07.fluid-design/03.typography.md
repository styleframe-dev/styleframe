...
The `useFluidFontSizeDesignTokens()` composable creates complete fluid typography systems that scale smoothly across all viewport sizes. It combines base font size ranges with modular scale multipliers to generate type hierarchies that maintain perfect proportions from mobile to desktop.
...
The `useFluidFontSizeDesignTokens()` composable generates a complete set of fluid font size variables based on base size ranges and modular scale multipliers.
...
**Pro tip:** Always call `useFluidViewportDesignTokens()` before using `useFluidFontSizeDesignTokens()`. The fluid font size composable relies on the fluid breakpoint variables created which are picked up automatically.

::

### Understanding the Parameters

The `useFluidFontSizeDesignTokens()` composable accepts three main parameters:
...
**3. Custom breakpoint** (optional) - By default, references the `fluidBreakpoint` variable from `useFluidViewportDesignTokens()`. You can override the default fluid breakpoint:

```ts
const { fluidBreakpoint } = useFluidViewportDesignTokens();

useFluidFontSizeDesignTokens(s, { min: 16, max: 18 }, sizes, customBreakpoint);
```
...
- **Always call `useFluidViewportDesignTokens()` first**: The other fluid composables depend on fluid breakpoint variables.
...
:::accordion-item{label="What's the difference between useFluidFontSizeDesignTokens() and useFluidClamp()?" icon="i-lucide-circle-help"}

- `useFluidFontSizeDesignTokens()` creates a complete typography system with multiple related sizes using modular scales. It's specifically designed for font sizes and handles scale power calculations. 

- `useFluidClamp()` creates a single fluid value between a min and max, and is more general-purpose for any CSS property. 

Use `useFluidFontSizeDesignTokens()` for typography systems, `useFluidClamp()` for individual values like spacing or border radius.
:::

:::accordion-item{label="Can I use different scales for different font sizes?" icon="i-lucide-circle-help"}
Yes! You can create multiple fluid font size systems with different scales. For example, use a subtle scale (1.125 to 1.25) for body text with `useFluidFontSizeDesignTokens()`, then create a separate system with a dramatic scale (1.333 to 1.5) for display text. Each system can have its own base size range and scale powers, giving you fine-grained control over different typography contexts.
:::
...
Yes! Mix `useFluidFontSizeDesignTokens()` for scalable sizes with `useFontSizeDesignTokens()` for fixed sizes. For example, use `useFluidFontSizeDesignTokens()` to create fluid heading sizes that scale smoothly, while using `useFontSizeDesignTokens()` for fixed body and UI text that stays consistent. This gives you the benefits of fluid scaling where it matters most (large display text) while maintaining predictability for smaller text.
:::
...
- **[Typography](/docs/theme/design-tokens/composables/typography)**: Explore fixed typography composables
