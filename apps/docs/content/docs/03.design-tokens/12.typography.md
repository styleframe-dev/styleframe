...
The `useFontFamilyDesignTokens()` function creates a set of font family variables from a simple object of font stack definitions.
...
const { fontFamily, fontFamilyBase, fontFamilyPrint, fontFamilyMono } =
...
const { fontFamily, fontFamilyMono, fontFamilySerif } =
	useFontFamilyDesignTokens(s, {
		default:
...
The `useFontSizeDesignTokens()` function creates a set of font size variables.
...
const { fontSize, fontSizeXs, fontSizeSm, fontSizeMd, fontSizeLg, fontSizeXl } =
...
The real power of `useFontSizeDesignTokens` comes when combined with `useMultiplierDesignTokens()` and modular scales. This allows you to create mathematically harmonious type systems:
...
:root {
	...
	--scale-power--4: calc(
		var(--scale) * var(--scale) * var(--scale) * var(--scale)
	);
	--scale-power--5: calc(
...
**Pro tip:** Using `useMultiplierDesignTokens()` with scales means you can change your entire type system's proportions by simply adjusting the scale ratio. Try different scales like Major Third (1.25) for more compact hierarchies or Perfect Fifth (1.5) for dramatic size differences.

::

## `useFontWeightDesignTokens`

The `useFontWeightDesignTokens()` function creates a set of font weight variables covering the standard range of weights.
...
The `useFontStyleDesignTokens()` function creates a set of font style variables for controlling text styling like italic and oblique.
...
const { fontStyle, fontStyleItalic, fontStyleSlanted } =
	useFontStyleDesignTokens(s, {
		default: "normal",
		italic: "italic",
...
The `useLineHeightDesignTokens()` function creates a set of line height variables for controlling vertical rhythm and text readability.
...
const { lineHeight, lineHeightHeading, lineHeightBody, lineHeightArticle } =
	useLineHeightDesignTokens(s, {
		default: 1.5,
		heading: 1.2,
		body: 1.5,
...
The `useLetterSpacingDesignTokens()` function creates a set of letter spacing (tracking) variables for fine-tuning text appearance.
...
// Font families
const { fontFamily, fontFamilyMono, fontFamilyDisplay } =
	useFontFamilyDesignTokens(s, {
		default:
...
// Line heights
const { lineHeightTight, lineHeightNormal, lineHeightRelaxed } =
	useLineHeightDesignTokens(s);
...
:root {
	...
	--font-size--2xl: calc(
		var(--font-size) * var(--scale) * var(--scale) * var(--scale)
	);
	--font-size--3xl: calc(
		var(--font-size) * var(--scale) * var(--scale) * var(--scale) * var(--scale)
	);
	--font-weight--normal: normal;
	--font-weight--medium: 500;
	--font-weight--semibold: 600;
...
- **Use modular scales for font sizes**: Combine `useFontSizeDesignTokens()` with `useMultiplierDesignTokens()` for mathematically harmonious type scales.
