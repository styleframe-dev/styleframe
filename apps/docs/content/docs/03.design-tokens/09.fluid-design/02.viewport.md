...
**Part of the Design Tokens Preset:** `useFluidViewportDesignTokens` is enabled by default in the [Design Tokens Preset](/docs/theme/design-tokens/presets) (`useDesignTokensPreset`) and you can configure it through the preset's `fluidViewport` option. For most projects, applying it via the preset is the recommended approach.
::
...
- **Use `useFontSizeDesignTokens()` for typography**: For complete type systems, pass `[min, max]` ranges to `useFontSizeDesignTokens()` instead of building individual `useFluidClamp()` calls. `useDesignTokensPreset` wires this up by default — pass `fluidFontSize: false` to opt out.
