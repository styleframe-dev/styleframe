...
:root {
	...
	--border-radius--xs: calc(
		var(--border-radius) * 1 / var(--scale) / var(--scale)
	);
	...
	--border-radius--2xl: calc(
		var(--border-radius) * var(--scale) * var(--scale) * var(--scale)
	);
}
```
...
const { borderRadius, borderRadiusSm, borderRadiusLg, borderRadiusFull } =
	useBorderRadiusDesignTokens(s, {
		default: "0.375rem",
		sm: "0.25rem",
		lg: "0.5rem",
