---
title: Border Radiuses Design Tokens
...
**Part of the Design Tokens Preset:** `useBorderRadiusDesignTokens` is included in the [Design Tokens Preset](/docs/theme/design-tokens/presets) (`useDesignTokensPreset`) and you can configure it through the preset's `borderRadius` option. For most projects, applying it via the preset is the recommended approach.
::
...
The `useBorderRadiusDesignTokens()` function creates a set of border radius variables from a simple object of radius value definitions.
...
## Integration with `useMultiplierDesignTokens`

The real power of `useBorderRadiusDesignTokens` comes when combined with `useMultiplierDesignTokens()` and modular scales. This allows you to create mathematically harmonious border radius systems that maintain consistent proportions.
...
**Pro tip:** Using `useMultiplierDesignTokens()` with scales means you can change your entire border radius system's proportions by simply adjusting the scale ratio. Try different scales like Major Third (1.25) for more dramatic differences or Major Second (1.125) for subtle variations.
...
- **Integrate with scales**: Combine `useBorderRadiusDesignTokens()` with `useMultiplierDesignTokens()` for mathematically harmonious border radius systems that scale proportionally.
...
:::accordion-item{label="What's the difference between useBorderRadiusDesignTokens and useMultiplierDesignTokens?" icon="i-lucide-circle-help"}
`useBorderRadiusDesignTokens()` creates border radius variables from explicit values you provide. `useMultiplierDesignTokens()` automatically generates border radius variables by multiplying a base border radius variable by scale powers. Use `useBorderRadiusDesignTokens()` for manual control or `useMultiplierDesignTokens()` for systematic, scale-based corner radius progressions.
:::
